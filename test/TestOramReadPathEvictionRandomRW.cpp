...
int* sampleDataRandReadPath(int i) {
...
TEST_CASE("Test OramReadPathEviction write and read randomly, very small numBlocks")
{	
	int bucketSize = 2;
	int numBlocks = 1;
    ...
    for(int i = 0; i < bound; i++){
        int acc = rand() % 2;
        if (acc == 0) {
            int* data = sampleDataRandReadPath(i%numBlocks);
...
TEST_CASE("Test OramReadPathEviction write and read randomly, larger numBlocks")
{   
    int bucketSize = 2;
    int numBlocks = 1024;
    Bucket::setMaxSize(bucketSize); 
...
TEST_CASE("Test OramReadPathEviction rewrite randomly and read, larger numBlocks")
{   
    int bucketSize = 2;
    int numBlocks = 1024;
    Bucket::setMaxSize(bucketSize); 
